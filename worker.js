// AugustDuarte File Portal — Cloudflare Worker entry point
// Env vars needed (set in Cloudflare Workers dashboard → Settings → Variables):
//   SITE_USER, SITE_PASS, JWT_SECRET
// R2 binding (configured in wrangler.toml):
//   FILES → augustduarte-files
...
const authed  = async (req, env) => { const t = getToken(req); return t ? verifyToken(t, env.JWT_SECRET) : false; };
...
const safeKey = raw => { const k = decodeURIComponent(raw); return (!k || k.includes('/') || k.includes('..')) ? null : k; };
